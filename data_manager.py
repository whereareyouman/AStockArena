...
from tools.json_file_manager import safe_update_json
...
"""
    ...
    def _batch_query_hour_data(self, c, symbols: List[str], begin_time: datetime, end_time: datetime) -> Dict[str, Optional[pd.DataFrame]]:
        ...
        for symbol in symbols:
            symbol_upper = str(symbol).upper()
            try:
                hour_df = self._ts_query_hour(c, symbol_upper, begin_time, end_time)
                result[symbol_upper] = hour_df
            except Exception as e:
                self.logger.warning(f"[TS] 批量查询小时线行情失败 {symbol_upper}: {e}")
                result[symbol_upper] = None
            # 批次间延迟，避免并发过多
            time.sleep(0.5)
        return result
    ...
    def _batch_query_hour_indicators(self, c, symbols: List[str], ndays: int) -> Dict[str, Optional[List[Dict[str, Any]]]]:
        ...
        for symbol in symbols:
            symbol_upper = str(symbol).upper()
            try:
                hour_ind = self._ts_query_indicators(c, symbol_upper, ndays=int(ndays), cycle='60m')
                result[symbol_upper] = hour_ind
            except Exception as e:
                self.logger.warning(f"[TS] 批量查询小时线指标失败 {symbol_upper}: {e}")
                result[symbol_upper] = None
            # 批次间延迟，避免并发过多
            time.sleep(0.5)
        return result
    ...
    def save_ts_data(self, symbols: List[str], ndays: int, out_path: str) -> bool:
        ...
        try:
            with self._ts_client_session() as c:
                ...
                begin_time = datetime.now() - timedelta(days=int(ndays))
