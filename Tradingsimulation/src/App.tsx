          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {DEFAULT_STOCK_SYMBOLS.map((stock) => {
                ...
                const price =
                  typeof overlay?.price === 'number' && overlay.price > 0 ? overlay.price : undefined;
                const changePercent =
                  typeof overlay?.changePercent === 'number' ? overlay.changePercent : undefined;
                const isPositive = typeof changePercent === 'number' ? changePercent >= 0 : false;
                const aiFocus =
                  typeof overlay?.aiHoldingCount === 'number'
                    ? overlay.aiHoldingCount
                    : typeof overlay?.aiAttention === 'number'
                    ? overlay.aiAttention
                    : undefined;
                const aiTrades = typeof overlay?.aiTradeVolume === 'number' ? overlay.aiTradeVolume : undefined;
                const hot = typeof aiFocus === 'number' && aiFocus >= 3;
