...
export function LeftStockSidebar({ onStockSelect, selectedStock }: LeftStockSidebarProps) {
  ...
  // periodically fetch live quotes and overlay onto defaults
  useEffect(() => {
    let cancelled = false;
  ...
  const renderStockItem = (stock: Stock) => {
    ...
    const aiFocus = stock.aiHoldingCount ?? stock.aiAttention ?? 0;
    const isHot = aiFocus >= 3;
    const aiTurnover = stock.aiTurnover ?? 0;
    const aiTrades = stock.aiTradeVolume ?? 0;
    ...
    return (
      ...
      >
        ...
        <div className={`flex items-center justify-between mt-1 text-xs ${aiVolumeClass}`}>
          ...
          <span className={aiTurnoverClass}>AI换手:{aiTurnover.toFixed(2)}%</span>
        </div>
